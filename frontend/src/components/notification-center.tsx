...
export default function NotificationCenter() {
  ...
  const loadNotificaciones = async () => {
    ...
    try {
      ...
      setNotificaciones(Array.isArray(response?.results) ? response.results : [])
      
      // Actualizar contador de no leídas
      const unread = response?.results?.filter((n: Notificacion) => !n.leida).length || 0
  ...
  const loadUnreadCount = async () => {
    try {
      const response = await api.getNotificacionesNoLeidas()
